...
from remotelogin.devices.exceptions import (
    UserAuthenticationValuesError,
...
class WithSlots:
    ...
    def check_args_from_dict(cls, cls_dict):
        if any([k not in cls.__slots__ for k in cls_dict]):
            raise KeyError(
                "{} only allowed the following keys: {}. You provided: {}"
...
class UserInfo(WithSlots):
    ...
    def get_kwargs(self, keys):
        return {
            k: getattr(self, k)
            for k in self.__slots__
            if k in keys and getattr(self, k) is not None
        }
...
class InterfaceInfo(WithSlots):
    ...
    def __init__(
        ...
        if not self.ip:
            raise ValueError(
                "You need to provide value for ip or hostname of the interface"
            )
...
class TunnelInfo(WithSlots):
    ...
    def _verify_hop(self, hop):
        ...
        if not host and proto in ("ssh", "telnet"):
            raise ValueError(
                "You need to specify all host values for each proxy jump doing ssh or telnet"
            )

        if proto not in self.PROTO_2_CONN_TYPES:
            raise ConnectionError(
                'The attribute "proto" of one of the hops should be one of {}'
        ...
        if proto == "command":
            if "cmd" not in hop:
                raise ConnectionError(
                    'A command should provide the "cmd" attribute to execute'
                )

            if "host" in hop or "port" in hop:
                raise ConnectionError(
                    'A command should not get a "host" or "port" attribute'
                )
        ...
        if user is None:
            ...
            raise ValueError(
                "For the time being We need a full user information or a UserInfo Object in hops"
            )
...
def _check_connection_arguments(conn_args, cls):
    if any([k not in cls.ARGUMENTS_ALLOWED for k in conn_args if k != "user"]):
        raise TypeError(
            "One or more of the keys/attributes ({keys_provided}) for the connection are not\n"
            "allowed for the connection type ({conn_type}).  \nAllowed attributes are: {allowed}"
            "".format(
                keys_provided=conn_args.keys(),
                conn_type=cls,
...
class ConnectionInfo:
    ...
    def __init__(
        self, proto, name, manager, user=None, interface=None, tunnel=None, **conn_args
    ):

        if proto not in KNOWN_CONNECTION_PROTOCOLS:
            raise NotImplementedProtocolError(
                "The proto ({}) is not an implemented protocol. "
        ...
        for user_arg in (
            ...
            if user_arg in conn_args:
                log.warning(
                    "This connection was passed a user argument ({}). We are dropping it to avoid any issues. "
                    "You need to add the user using device.users object".format(
                        user_arg
                    )
                )
        ...
        if isinstance(default_value, cls):
            if default_value not in manager_instance:
                ...
                raise ValueError(
                    ...
                    " values please define it correctly.".format(
                        item_name=cls.__name__, name=default_value.name
                    )
                )
            return default_value

        raise ValueError(
            "The value provided for {} cannot be converted to the proper class. Check attributes"
            "".format(cls.__name__)
        )
        ...
        if user:
            ...
            if not user:
                raise NoDefaultUserError(
                    "you need to provide a user or set the device or "
    ...
    def _set_conn_interface_args(self, interface, conn_args):
        ...
        if interface:
            ...
            if not interface:
                raise NoDefaultInterfaceError(
                    "you need to provide an interface or set the device or "
        ...
        conn_args["host"] = (
            interface.default_ipv6 if ip_type == "ipv6" else interface.default_ip
        )
    ...
    def new_open_instance(self, user, instance_name, tunnel, interface, **conn_kwargs):
        ...
        instance_name = self.manager._check_instance_name_not_taken_and_open(
            self.name, instance_name, user, tunnel, interface
        )
    ...
    def data_conversations_iterator(self, instance_names=()):
        for instance_name, instance_conversations in (
            (name, v)
    ...
    def conversations(self, instance_names=()):
        ...
        for instance_name, conversation in self.data_conversations_iterator(
            instance_names
        ):
...
KNOWN_CONNECTION_PROTOCOLS = dict(
    ssh=lambda: terminal.TerminalConnectionWrapper(ssh.SshConnection),

...
def is_sequence(arg):
    return (
        ...
        and (
            isinstance(arg, collections.Sequence)
            or isinstance(arg, collections.Mapping)
        )
    )
...
def is_list_or_tuple(data):
    return (
        data is not None
        and isinstance(data, collections.Sequence)
...
def _add_elements_to_list(
    original_list, elements, condition, count, unique, raise_exception_if_not_unique
):
...
def add_to_list(
    to_list, new_elements, unique=False, count=0, raise_exception_if_not_unique=False
):
    return _add_elements_to_list(
        to_list, new_elements, None, count, unique, raise_exception_if_not_unique
    )


def add_to_list_if_condition(
    to_list,
    elements,
    condition,
    unique=False,
    count=0,
    raise_exception_if_not_unique=False,
):
    return _add_elements_to_list(
        to_list, elements, condition, count, unique, raise_exception_if_not_unique
    )
...
def sort_nested_dict_generator(
    sort_me, item_key, start_return=0, count=0, condition=lambda x: x, reverse=False
):
    counter = 0
    for key in sorted(
        sort_me.keys(), key=lambda x: condition(sort_me[x][item_key]), reverse=reverse
    ):
        counter += 1
...
def sort_nested_dict(
    sort_me, item_key, start_return=0, count=0, condition=lambda x: x, descending=False
):
    ...
    for key in sorted(
        sort_me.keys(),
        key=lambda x: condition(sort_me[x][item_key]),
        reverse=descending,
    ):
        counter += 1
        if start_return and counter < start_return:
...
def list_to_matrix_index_generator(values, rows=None, columns=None, row_order=True):
    ...
    if rows and columns and (rows * columns) < values_length:
        raise ValueError(
            "The number of rows by columns does not match the number of elements in values"
        )
...
def print_in_columns(values, columns=5, column_width=20, center=True, row_order=False):
    ...
    string_format = lambda v: "{v:{align}{column_width}}".format(
        v=v, align=align, column_width=column_width
    )

    for row_values in list_to_matrix_index_generator(
        values, columns=columns, row_order=row_order
    ):
